...
int make_dir(tree_p current_dir) {
	...
	if (NULL == current_dir->children) {
		...
		tree_p q = (tree_p)malloc(sizeof(tree_p));
		...
		/*sorted input*/
		if (strcmp(current_dir->children->directory, q->directory) > 0) {
			insert_before(q, current_dir);
			strftime(s, 1000, "%d/%m/%Y  %H:%M:%S", p);
			strcpy(q->time, s);
		}
		else {
			while (sub_dir->siblings != NULL && strcmp(q->directory, sub_dir->siblings->directory) > 0) {

				if (strcmp(q->directory, sub_dir->directory) == 0) {
					printf("A subdirectory or file %s already exists.\n", q->directory);	//problem kod duplikata, sub_dir->siblings se zanemari kad je jedan element
					free(q);
					return 1;
				}
...
tree_p change_dir(tree_p current_dir, position head, tree_p root_address) {
	...
	{
		push(current_dir, head, root_address);
