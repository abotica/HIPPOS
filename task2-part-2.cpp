...
//operator > opterećujemo kao vanjsku funkciju
//možemo ga preopteretiti kao vanjsku ali i kao člansku funkciju
...
//operator za čitanje sa toka
//prima jedan tok za ulazni tok i refernecu na objekt, a vraca ulazni tok
istream& operator>>(istream& istr, Razlomak& me) {

    int b, n;

    //čitanje brojnik i nazivnik
    istr >> b >> n;
    if (istr) {
        me.m_brojnik = b;
        //provjera nazivnika je već napisana u SetNazivnik() pa je zato samo pozivamo
        me.SetNazivnik(n);  //ovaj n je prosljeđen u f-ju kao 'naz' te se provjerava je li nazivnik nula 
    }
...
Razlomak& Razlomak::operator=(Razlomak& other) {
    ...
    //on još mora vratiti (de)referencirani pokazivač na samog sebe tako da se može nadovezivati na operator =
    m_brojnik = other.m_brojnik;
    m_nazivnik = other.m_nazivnik;

    //operator = nam je član klase stoga možemo dirati privatne varijable, a da nije motali bi koristiti set i get funkcije
...
int main() {
    ...
    //dinamička alokacija ovako...  ili kao niz od dva elementa
...
int main() {
    ...
    //ono što ćemo pisati
    //ako je zadano da se dodaje uključujemo append!
    ofstream ofs("poz.num", ios::app);

    double d;

    //pročitamo iz ulaznog toka varijablu i onda nam na tom mjestu ostaje ifs - ulazni tok; ako je true sve ok, a ako je false dosli smo do kraja datoteke
    while (ifs >> d) {
        if (d >= 0) {           //može i samo >
            ofs << d << endl;
        }
...
class Point4D : public Point2D {
    ...
    public:
        ...
        //konstruktori se izvode po liniji nasljeđivanja -> prvo konstruktor temeljne, a onda konstruktor nasljeđene
        //U konstruktoru nasljeđene klase treba napisati samo dodatne varijable
        Point4D() { m_z = m_t = 0.0; }
};


//operator nije jednako radimo kao vanjsku funkciju i vraća bool
bool operator!=(Point4D & left, Point4D & right) {
    ...
    if (left.m_z != right.m_z)
        return true;
    if (left.Get_t() != right.Get_t())   //ne možemo direktno stavit m_t jer samo u vanjskoj funkciji (nije članska)
        return true;
    return false;
}

//operator za čitanje -> prima dvije referencu 
istream& operator >>(istream& istr, Point4D& me) {
    double a, b, c, d;   //privremene varijable koje čitamo
...
int main() {
    ...
    cout << (a != b) << endl;  //oboje su na početku nula, što znači da su jednaki (false) ispisuje se nula
    cin >> a;
    cout << (a != b) << endl;
...
int main() {
    ...
    list<double> r;                             //povratna lista koja je prazna na početku
...
// 
// NAPOMENA: Funkcija za vađenje drugog korijena sqrt je dio biblioteke <cmath>
// 
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// Ne mijenjajte nista iznad ove linije!
...
// - operator >> koji sa toka čita najprije ime, a potom ocjenu 
//   (operator neka bude vanjska friend funkcija tipa ulaznog toka,
//    sa prvim argumentom objektom ulaznog toka sa kojeg se čita,
//    te drugim argumentom objektom klase CUcenik u koju se čita)
// 
// U main() funkciji dinamički deklarirajte dva objekta klase CUcenik, 
// ucitajte njihove podatke sa tipkovnice pomoću operatora >>,
...
// Napravite klasu GMOBIljka, koja nasljeđuje od klase Biljka,
// a ima dodatnu člansku varijablu imena GMO, tipa bool.
// 
...
//  d) operator == koji neka bude članska funkcija koja prima
...
void Razlomak::SetNazivnik(int naz) {
    if (naz != 0) {
        ...
        cout << "Nedozvoljena vrijednost nazivnika --> nazivnik različit od nule!" << endl;
    }
}
